...
from model.model_functions import iter_loadtxt

dunnhumby = iter_loadtxt("./data_prep/dunnhumby/transaction_data.csv", delimiter=',', skiprows=1).astype('int64') # csv file can be downloaded from Kaggle
...
trunc = True # Can be set to True to obtain predictions based on only the 50 most recent baskets (similar to TARS)
if trunc:
  maxlen = 51
  nusers = np.max(dunnhumby_coord[:,0])+1
